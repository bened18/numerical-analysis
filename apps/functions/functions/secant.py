...
def secant(function, p_0, p_1, tol, n):
    ...
    while i <= n:
        ...
        i+=1;
    if i > n:
        return(f"solution not found for tolerance: {tol} spend iterations: {i-1}", tabulate(resultados, headers=["iter", "Xi", "f(xi)", "error"], tablefmt="html",floatfmt=(".10f",".10f")))   
    if i < n:
        return (f"Approximation of root found in x = {p_2}",tabulate(resultados, headers=["iter", "Xi", "f(xi)", "error"], tablefmt="html",floatfmt=(".10f",".10f"))) 
